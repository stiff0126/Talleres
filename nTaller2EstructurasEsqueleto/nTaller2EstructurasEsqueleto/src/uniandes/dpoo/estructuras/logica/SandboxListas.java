...
public class SandboxListas
{
    ...
    public void eliminarCadena( String cadena )
    {
    	for (int j = 0; j < listaCadenas.size(); j++) {
			if (listaCadenas.get(j)==cadena) {
				listaCadenas.remove(j);
			}
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
    	for (double valor : valores) {
            int enteroTruncado = (int) Math.floor(valor);
    ...
    {
    	List<String> listaCadenas1= new ArrayList<String>( );
    	for (Object objeto : objetos) {
            listaCadenas1.add(objeto.toString());
        }
    	
    	listaCadenas=listaCadenas1;
    }
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (int i = 0; i < listaEnteros.size(); i++) {
            ...
            for (int j = 0; j < listaEnteros.size(); j++) {
                if (i != j && elementoActual == listaEnteros.get(j)) {
                    repetido = true;
                    break;
                }
            }
